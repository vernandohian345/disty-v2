...
export default function MainStats({ stats }) {
  const statsData = [
...

    {
      ...
      icon: <Newspaper size={28} className="text-orange-600" />,
    },

    {
      title: "Peserta Pelatihan",
      value: stats.total_peserta_pelatihan,
      link: "/admin/Pembayaran",
...

    {
      ...
      value: stats.total_peserta_sertifikasi,
      link: "/admin/transaksi-sertifikasi",
      bgColor: "bg-gradient-to-br from-cyan-100 to-cyan-200",
      icon: <Award size={28} className="text-cyan-600" />,
    },

    {
      title: "Belum Generate",
      value: stats.sertifikat_belum_generate,
      link: "/admin/sertifikat",
      bgColor: "bg-gradient-to-br from-yellow-100 to-yellow-200",
      icon: <FileBadge size={28} className="text-yellow-600" />,
    },
  ];
      ...
      <div className="xl:hidden overflow-x-auto pb-2">
        <div className="grid grid-rows-2 grid-flow-col gap-3 w-max">
          {statsData.map((item, index) => (
            <div key={index} className="w-[145px]">
              <StatsCard
                title={item.title}
                value={item.value}
