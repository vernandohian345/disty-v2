...
// ── ICONS ──────────────────────────────────────────────────
// Kita gunakan set ikon yang sama dengan BootcampHero untuk konsistensi visual
const ClockIcon = () => (
  <svg
    className="w-4 h-4"
    fill="none"
...
// ── SKELETON ───────────────────────────────────────────────
// Menyesuaikan teks 'Bootcamp' menjadi 'Sertifikasi'
function SkeletonHero() {
  return (
    <section className="relative overflow-hidden bg-[#0f0a07]">
      <div className="max-w-[1800px] mx-auto px-4 lg:px-6 py-6">
        <div className="relative overflow-hidden rounded-[36px] min-h-[680px] bg-[#fffaf5] border border-white/[0.07] animate-pulse">
...
// ── NOT FOUND ──────────────────────────────────────────────
// Menyesuaikan teks 'Pelatihan' menjadi 'Sertifikasi'
function NotFound() {
  return (
    <section className="relative overflow-hidden bg-[#0f0a07]">
      <div className="max-w-[1800px] mx-auto px-4 lg:px-6 py-6">
        <div className="relative overflow-hidden rounded-[36px] min-h-[480px] bg-[#1a0f0a] border border-white/[0.07] flex items-center justify-center">
          ...
          <div className="relative z-10 text-center px-6">
            ...
            <p className="text-white/45 text-sm mb-8">
              Program sertifikasi yang kamu cari tidak tersedia atau telah
              dihapus.
            </p>
            <Link
              to="/sertifikasi"
...
export default function CertificationDetailHero() {
  ...
  const fetchCertification = async () => {
    try {
      setLoading(true);
      // Asumsi endpoint API sertifikasi. Sesuaikan jika berbeda.
      const response = await axios.get(
        `http://127.0.0.1:8000/api/frontend/sertifikasi/${slug}`,
      );
      setCertification(response.data.sertifikasi);
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  const isFree =
    Number(certification.harga) === 0 || certification.kategori === "gratis";
  const price = isFree
    ? "Gratis"
    : `Rp ${Number(certification.harga).toLocaleString("id-ID")}`;

  // Feature tags — use certification.tools if API returns it
  const featureTags =
    Array.isArray(certification.tools) && certification.tools.length
      ? certification.tools
      : null;
  ...
  const metaItems = [
    {
      ...
      label: "Durasi Masa Berlaku",
      value: certification.durasi, // Asumsi nama field dari API
    },
    {
      icon: <BarIcon />,
  ...
  const enrollRows = [
    { label: "Durasi Ujian", value: certification.durasi }, // Asumsi nama field

    { label: "Format", value: certification.format_ujian || "Online" },
    { label: "Sertifikat Resmi", value: "✓ Tersedia" },
  ];
      ...
      <div className="max-w-[1800px] mx-auto px-4 lg:px-6 py-6">
        ...
        >
          ...
          <div className="absolute inset-0">
            <img
              src={certification.sampul_url}
              alt={certification.nama_sertifikasi}
            ...
            <div className="flex-1 max-w-[580px] pt-8">
              ...
              <div className="inline-flex items-center gap-2 bg-white/[0.08] backdrop-blur-xl border border-white/[0.1] rounded-full px-4 py-2 mb-6">
                ...
                <span className="text-white/80 text-[11px] font-bold uppercase tracking-[0.1em] ">
                  {certification.kategori}
                </span>
              </div>

              {/* Title — 3-line mixed treatment (Cenderung putih) */}
              <h1
                className="font-white leading-[1.01] tracking-tight mb-5 text-white"
                style={{ fontSize: "clamp(2.2rem, 4.5vw, 3.8rem)" }}
              >
                {/* Attempt to split title for visual interest */}
                {(() => {
                  const words = certification.nama_sertifikasi.split(" ");
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Link
                  to={`/checkout/sertifikasi/${certification.slug}`} // Sesuaikan route checkout
                  className="
                  ...
                  "
                >
                  <span>Daftar Ujian Sekarang</span>
                ...
                <a
                  href={`https://wa.me/6281234567890?text=Halo Disty Academy, saya ingin bertanya tentang sertifikasi ${encodeURIComponent(certification.nama_sertifikasi)}`}
                  target="_blank"
              ...
              >
                <p className="text-[9px] text-white/30 uppercase tracking-[0.12em] font-bold mb-3">
                  Manfaat Tersertifikasi
                </p>
                {[
                  "Validasi keahlian industri",
                  "Meningkatkan daya saing global",
                  "Sertifikat fisik & digital",
                  "Akses komunitas profesional",
                ].map((item) => (
                  <div
                    key={item}
